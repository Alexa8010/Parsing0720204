...
class ImpactfactorSpiderSpider(scrapy.Spider):
    name = "impactfactor_spider"
    allowed_domains = ["scimagojr.com"]
    start_urls = ["https://scimagojr.com/journalrank.php?category=2705&area=2700&country=Western%20Europe&order=sjr&ord=desc"]
    ...
    def parse(self, response):
        ...
        for row in rows:
            ...
            yield response.follow(url=link, callback=self.parse_impact,  meta={
                                      'title': title,
                                      'type' : type,
    ...
    def parse_impact(self, response):
        rows = response.xpath('/html/body/div[7]')
        for row in rows:
            title = response.request.meta['title']
            country = response.xpath('.//div/div/div[1]/p/a[1]/text()').get()
